...
# asked whether AI takes over the work the knowledge is applied in. Measured on
# 59 knowledge items against the Gemini scores the effect is small: mean gap
# 1.31 -> 1.13 on the 1-10 scale, rank correlation 0.76 -> 0.77.
...
def main():
    ...
    parser.add_argument("--rps", type=float, default=15.0,
                        help="Request starts per second (API cap is 1,200/min)")
            ...
            for future in as_completed(futures):
                ...
                if done % CHECKPOINT_EVERY == 0:
                    ...
                    print(
                        ...
                        f"${tokens * PRICE_PER_MTOK / 1e6:.3f}, "
                        f"errors {len(errors)})",
                        flush=True,
                    )
    finally:
